...
export function AppToolbar({
  ...
  const currentInst = institutions.find((n) => n.id === institutionId) || institutions[0];
  ...
  const handleCalculate = () => {
    if (stations.length === 0)
      return addNotification("يرجى إضافة محطة تعبئة أولاً", "warning");
    if (points.length === 0)
      return addNotification("يرجى إضافة نقاط توزيع أولاً", "warning");
    const totalInstitutions = stations.reduce(
      (sum, s) => sum + s.institutions.length,
      0,
    );
    ...
    if (result.unservedPoints.length > 0) {
      addNotification(
        `${result.trips.length} رحلة | عجز: ${result.unservedPoints.length} نقطة`,
        "warning",
      );
    } else {
  ...
  return (
    <header
      style={{
        background:
          "linear-gradient(135deg, #0a1628 0%, #122240 50%, #0a1628 100%)",
    ...
    >
        ...
        {user && (
          <div
            style={{
              ...
              fontSize: "0.72rem",
              fontWeight: 700,
              background: isAdmin ? "rgba(239,68,68,0.15)" : (currentInst ? `${currentInst.color}25` : "rgba(59,130,246,0.15)"),
              color: isAdmin ? "#f87171" : (currentInst ? currentInst.color : "#3b82f6"),
              border: `1px solid ${isAdmin ? "rgba(239,68,68,0.3)" : (currentInst ? `${currentInst.color}40` : "rgba(59,130,246,0.3)")}`,
              display: "flex",
              alignItems: "center",
              gap: 6,
            }}
          >
            ...
            <span>{isAdmin ? 'مدير النظام' : (isNGO ? 'منسق' : 'سائق')}</span>
            {currentInst && !isAdmin && (
              <>
                <span style={{ opacity: 0.6 }}>|</span>
                <span>{currentInst.name}</span>
              </>
            )}
          </div>
        )}
      ...
      {/* ═══ Active Mode Indicator (admin only) ═══ */}
      {isAdmin && mode && (
        <div
          style={{
            position: "absolute",
